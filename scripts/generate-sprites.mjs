...
const canvas = 64;
const labCropBoxes = {
  cto: { x: 150, y: 68, width: 300, height: 330 },
  pm: { x: 626, y: 70, width: 296, height: 326 },
  frontend: { x: 1096, y: 70, width: 314, height: 326 },
...
function cropPng(sourcePng, box) {
  const png = new PNG({ width: box.width, height: box.height });
  png.data.fill(255);
  ...
  for (let y = 0; y < box.height; y += 1) {
    for (let x = 0; x < box.width; x += 1) {
      ...
      png.data[dstIdx] = sourcePng.data[srcIdx];
      png.data[dstIdx + 1] = sourcePng.data[srcIdx + 1];
      png.data[dstIdx + 2] = sourcePng.data[srcIdx + 2];
      png.data[dstIdx + 3] = sourcePng.data[srcIdx + 3];
    }
  }
